...
APKTOOL_UPGRADE() {
    ...
    wget -O /usr/bin/apktool https://raw.githubusercontent.com/iBotPeaches/Apktool/master/scripts/linux/apktool &>/dev/null ; wait && wget -O /usr/bin/apktool.jar https://bitbucket.org/iBotPeaches/apktool/downloads/apktool_2.4.0.jar &>/dev/null ; wait && echo && echo -e "${YELLOW}Download complete ${GREEN}apktool ${YELLOW}wrapper and ${GREEN}apktool.jar ${YELLOW}are in /usr/bin"
